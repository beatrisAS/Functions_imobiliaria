...
CREATE PROCEDURE AtualizarValorApartamento (
  IN condominio_id INT,
  IN percentual_aumento DOUBLE
)
BEGIN
  UPDATE apartamento
  SET valor = valor + (valor * percentual_aumento / 100)
  WHERE codigo_cond = condominio_id;
END $$

DELIMITER ;


SELECT * FROM apartamento WHERE codigo_cond = 1; -- Antes da execução

CALL AtualizarValorApartamento(1, 10); -- Execução

SELECT * FROM apartamento WHERE codigo_cond = 1; -- Depois da execução
...
SELECT * FROM proprietario_apartamento WHERE numero_ap = 'b301'; -- Antes da execução

CALL EfetivarCompraApartamento('54367281-2', 'b301'); -- Execução

SELECT * FROM proprietario_apartamento WHERE numero_ap = 'b301'; -- Depois da execução
...
CREATE FUNCTION obter_nome_sindico (id_sindico INT)
RETURNS VARCHAR(80)
DETERMINISTIC
BEGIN
...
SELECT 
  ...
  obter_nome_sindico(c.matricula_sind) AS nome_sindico
FROM condominio c;
...
CREATE FUNCTION calcular_taxa_condominio (id_apartamento VARCHAR(5), percentual DOUBLE)
RETURNS DOUBLE
DETERMINISTIC
BEGIN
  DECLARE valor_apartamento DOUBLE;
  DECLARE taxa DOUBLE;

  SELECT valor INTO valor_apartamento
  FROM apartamento
  WHERE numero = id_apartamento;

  SET taxa = valor_apartamento * (percentual / 100);
...
SELECT 
  ...
  calcular_taxa_condominio(a.numero, 2) AS taxa_condominio
FROM apartamento a
